 ...
 */
Route::{$dataTypeL}(
    ':v/zhifu_module/{$name}_route',
    'zhifu_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/zhifu_route_v1_api.php")){
            file_put_contents(
                '../route/zhifu_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
